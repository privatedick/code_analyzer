...
# Definiera mappstrukturen och filerna med innehåll
project_structure = {
    "src/code_analyzer": {
        "code_analysis.py": """
import os
...
def test_analyze_project(analyzer):
    with patch('src.code_analyzer.analyzer.CodeAnalysis') as mock_code_analysis:
...
def test_compare_projects(analyzer):
    with patch('src.code_analyzer.analyzer.ProjectComparison') as mock_project_comparison:
...
def test_visualize_results(analyzer):
    with patch('src.code_analyzer.analyzer.Visualization') as mock_visualization:
...
def test_main_analyze():
    with patch('argparse.ArgumentParser.parse_args') as mock_args, \
         patch('src.code_analyzer.config.Config') as MockConfig, \
         patch('src.code_analyzer.analyzer.CodeAnalyzer') as MockCodeAnalyzer:
...
def test_compare_projects(project_comparison, mock_projects):
    with patch('src.code_analyzer.project_comparison.CodeAnalysis') as MockCodeAnalysis:
...
def test_create_complexity_heatmap(mock_heatmap, mock_close, mock_savefig, visualization, mock_analysis_results):
    ...
    mock_savefig.assert_called_once_with('complexity_heatmap.
